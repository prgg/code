...
function selectBar(config) {
	...
	*/
	this.log = {
		time: "2013.5.23",
		version: "\n
			2.013	新增：地区名称列表数据(CRM)\n
			2.013	新增：行业名称列表数据(CRM)\n
			2.012	新增：职场栏目数据(CRM)\n
	...
	*/
	...
	this.add = function(val){
		return this.form.popInput.add({
			"val" : val.toString()
		});
	};
	//外部接口：删除选中项目
	this.remove = function(val){
		this.form.popInput.remove({
			"val" : val.toString()
		});
	};	
	//外部接口：删除全部选中项目
	...
	//外部接口：获取已选中对象code值的集合
	this.getCheckedCodes = function(){
		return this.form.popHideVal.get();
	};
// 	原有弹出层修改
// 		add方法返回bool
// 		获取已经选中的code集合getCheckedCodes()
// 		nameObj参数是null或空的时候，关闭弹出层不写text
// 		弹出层关闭增加回调方法
// 提供json数据格式
...
function formData(cfg) {
	...
	this.obj = {
		...
		_getId : function(val){
			if(val == "all")
				return [val];
			val = val.replace("v2_","");
			var data = qz.getParentsByCode(cfg.cont.title + "_" + cfg.cont.lan,val),
				dataLen = data.length;
			var codeVal = [];
			if(dataLen){
				for(var i = 0; i < dataLen; i++){
					if(data[i].constructor === Array)
						codeVal.push(data[i].join("_"));
					else
						codeVal.push(data[i]);
				};
			}
	...
	this.cookieObj = (function() {
		...
		return {
			...
			set: function(val) {
				...
				var valOld = this.get() || "";console.log(domain);
				if (valOld != "") {
					//如果传入值与已存在值一致则终止执行
	...
	this.popInput = {
		...
		run: function(obj) {
			var val = obj.value,
				in_arr = !(new RegExp(val).test(THIS.popHideVal.get().join()));
			if (obj.checked && in_arr){
				this.add({
					"val": val
				});
			}
		...
		//添加
		add: function(conf) {
			THIS.init.showList(conf.val);
			var obj = THIS.obj.getCheckbox(conf.val),
				getNumMax = this.getNumMax();
			if (getNumMax) {
	...
	this.init = {
		showList: function(val) {
			if(val.constructor !== Array){				
				var id = THIS.obj.getId(val);
				if(THIS.obj.getCheckbox(val,true).length != 0 && id.length == 1)
					return true;
				if(THIS.obj.getCheckbox(val).length != 0 && id.length == 1){
				...
				for(;i < len; i++){
					var checkbox = THIS.obj.getCheckbox(val[i],true);
					if(!checkbox){
						var checkbox2 = THIS.obj.getCheckbox(val[i]);
						if(checkbox2.length){
							THIS.obj.getLi(val[i]).attr("i", 0).click();
						}
					}
					else{
						THIS.obj.getLi(val[i]).attr("i", 0).click();
		...
		popOldList_cookie: function() {
			...
			var valArr = val.split("|").reverse(),
				len = valArr.length;
			for (var i = 0; i < len; i++) {
				if(THIS.obj.getCheckbox(valArr[i],true).length == 0)
					this.showList(valArr[i]);
		...
		pop_form: function() {
			...
			if (viewInput && viewInput[0] != false && hideInput && hideInput[0] != false) {
				...
				for (var i = 0; i < hideInput.length; i++) {
					if(THIS.obj.getCheckbox(hideInput[i],true).length == 0)
						this.showList(hideInput[i]);
...
function popDiv(config) {
	...
	var c = {
		...
		bg: {
			...
			opacity: config.bg && config.bg.opacity || 0.5
		},
		closeBut: {
	...
	this.createBgTag = function() {
		...
		//if (!-[1, ] && !window.XMLHttpRequest) {
			bgDiv.html("<iframe src='about:blank' frameborder='0' width='9999' height='9999' scrolling='no' style='filter:alpha(opacity=0);opacity:0;width:9999px;height:9999px;'></iframe>");
		//}
		bgDiv.appendTo("body");
	};
	//弹窗内容载体容器 创建
	...
	this.showDiv = function() {
		...
		if ($("#" + c.cont.id).length) {
			...
			$("#" + c.cont.id).css("z-index", createBackgroundDom_zIndex);
		}
	};
	//弹窗容器 隐藏
			...
			if(data[i] != undefined){
				...
				if(o.lv == 1) {
					...
					$("<li>", {
						"class" : childrenClass,
						"title" : data[i][k.name],
						html: "<strong><span>" + data[i][k.name] + "</span></strong>"
